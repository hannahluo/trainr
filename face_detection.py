...
import cv2, sys, smtplib, time, base64, socket, serial
...
server_address = ('10.42.0.68', 10001)
...
face_cascade = cv2.CascadeClassifier('/home/linaro/opencv/data/haarcascades/haarcascade_frontalcatface.xml')
extended_face_cascade = cv2.CascadeClassifier('/home/linaro/opencv/data/haarcascades/haarcascade_frontalcatface_extended.xml')
...
while(True):
    ...
    if len(faces) > 0 and cur_time - last_time >= IMAGE_CAPTURE_TIMEOUT:
        ...
        encoded_string = b"Cat detected!" #base64.b64encode(image_file.read())
        time.sleep(3)
        #print(str(encoded_string[len(encoded_string) - 10:len(encoded_string)]))
